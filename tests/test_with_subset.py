...
def test_preprocess_data():
    ...
    data = pd.read_csv("project/data/amazon_reviews_subset.csv")
    processed_data = preprocess_data(filepath="project/data/amazon_reviews_subset.csv",
                                     text_column="review_text",
                                     numeric_columns=["rating"])
    # Vérification que les colonnes essentielles sont intactes
    assert "review_text" in processed_data.columns
...
def test_kl_divergence():
    # Chargement du sous-ensemble
    data = pd.read_csv("project/data/amazon_reviews_subset.csv")
    train_data = data.iloc[:3]  # Utilisation des 3 premiers exemples pour entraîner
    test_data = data.iloc[3:]   # Utilisation du reste pour tester
